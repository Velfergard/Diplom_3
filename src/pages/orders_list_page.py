...
class OrdersListPage(AccountPage):
    ...
    def click_on_order_from_orders_list(self):
        self.driver.find_element(*locators.ORDER_ELEMENTS).click()


    @allure.step("Проверяем, что открылось окно с информацией о заказе")
    def check_order_details_is_opened(self):
        order_window = self.driver.find_element(*locators.ORDER_WINDOW).get_attribute("class")
    ...
    def check_order_from_pa_is_shown(self, order_id):
        orders = self.driver.find_elements(*locators.ORDERS_IDS)
        ids = []
    ...
    def collect_orders_all_counter_value_before_order(self):
        counter = self.driver.find_element(*locators.ORDERS_ALL_COUNTER).text
    ...
    def check_orders_all_counter_value_after_order(self, counter):
        ...
        counter_after = self.driver.find_element(*locators.ORDERS_ALL_COUNTER).text
    ...
    def collect_orders_today_counter_value_before_order(self):
        counter = self.driver.find_element(*locators.ORDERS_TODAY_COUNTER).text
    ...
    def check_orders_today_counter_value_after_order(self, counter):
        counter_before = counter
        counter_after = self.driver.find_element(*locators.ORDERS_TODAY_COUNTER).text
    ...
    def check_order_id_in_orders_in_progress(self, order_id):
        orders = self.driver.find_elements(*locators.ORDER_IN_PROGRESS)
        ids = []
